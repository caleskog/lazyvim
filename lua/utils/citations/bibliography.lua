...
local path_utils = require("utils.citations.path")
  ...
  for idx, line in ipairs(lines) do
    ...
    elseif in_front_matter then
      ...
      if collecting_list then
        ...
        if item then
          results[#results + 1] = { name = ensure_bibliography_extension(trim(item)), line = idx }
        end
      end
    end
  end
  ...
  for idx, line in ipairs(lines) do
    for _, pattern in ipairs({
      ...
      for path in line:gmatch(pattern) do
        path = ensure_bibliography_extension(trim(path))
